...
REM Compress with PowerShell gzip
echo Compressing backup...
powershell -nologo -noprofile -command "$in=[System.IO.File]::OpenRead('!BACKUP_FILE!'); $out=[System.IO.File]::Create('!BACKUP_FILE!.gz'); $gz=New-Object System.IO.Compression.GZipStream($out,[System.IO.Compression.CompressionMode]::Compress); $in.CopyTo($gz); $gz.Close(); $out.Close(); $in.Close()"
